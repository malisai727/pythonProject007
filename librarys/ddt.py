...
#备注：这里的ddt文件是从源码那里拷贝过来做了修改的，详见286行被修改代码的注释---dahui
    ...
    for name, func in list(cls.__dict__.items()):
        if hasattr(func, DATA_ATTR):
            for i, v in enumerate(getattr(func, DATA_ATTR)):
               ...
               #test_data_docstring = _get_test_data_docstring(func, v) #该行被注释的是源码
                test_data_docstring = v.title#author:dahui----该行是自己加的代码，v相当于是传入的测试数据，title是数据里的字段，这里是把用例title加入，显示在report里（注意先要把数据取出来，用封装的excel类）
